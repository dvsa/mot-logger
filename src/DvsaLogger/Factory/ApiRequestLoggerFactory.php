...
/**
 * Class ApiRequestLoggerFactory
 *
 * @package DvsaLogger\Factory
 */
class ApiRequestLoggerFactory implements FactoryInterface
{
    /**
     * @var  Logger
     */
    ...
    public function __invoke(ContainerInterface $container, $requestedName, array $options = null)
    {
        ...
        $tableName = $config['listeners']['api_request']['options']['tableName'];
        $columnMap = $config['listeners']['api_request']['options']['columnMap'];

        $writer = new DbWriter($adapter, $tableName, $columnMap);
        $writer->setFormatter(new DbFormatter('Y:m:d H:i:s'));
        $this->logger->addWriter($writer);

        $processor = $container->get('DvsaLogger\ApiRequestExtras');
